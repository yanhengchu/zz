...
3. 页面同步当前状态；只有检测到确实有运行中的周期任务、OCR 或连点时，才统一停止

### 启动任务流程

1. 用户在 `MainActivity` 中点击“开始上滑”
...
- 上滑场景当前提供 3 秒和 50 秒两个固定节奏入口

### 连点流程

当前连点能力保留在代码中，但 `MainActivity` 入口按钮暂隐藏。

1. 入口恢复后，用户在 `MainActivity` 中点击“连点”
2. 页面显示独立连点悬浮按钮，初始状态为 `off`
3. 用户点击连点悬浮按钮，按钮切换 `on/off`
4. `GestureService` 接收 `ACT_TOGGLE_CONTINUOUS_CLICK` 并启动或停止 `ContinuousClickRunner`
5. runner 每 50 ms 读取连点悬浮按钮位置，并复用点击任务的锚点算法执行点击
6. `ACT_STOP` 会停止当前连点，并把悬浮按钮状态更新为 `off`

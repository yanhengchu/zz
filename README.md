...
- 用户操作：
  用户打开操作页，点击第一个“开始上滑”启动 3 秒节奏任务，或点击第二个“开始上滑”启动 30 秒节奏任务。
  任务运行过程中，双击悬浮窗可回到操作页并结束当前任务。
- 系统执行：
  任务启动后先等待 5 秒，再执行第一次上滑。
  每完成一次上滑后，按入口配置固定等待 3 秒或 30 秒进入下一轮，并持续循环。
  悬浮窗展示当前轮次剩余时间。
- 关键约束：
  打开操作页视为停止当前任务。
  “固定 3 秒”和“固定 30 秒”指每次上滑后的下一轮等待，不和首次启动前的固定 5 秒等待混用。
  执行依赖无障碍服务可用。

...
- 操作页：
  ...
  - 固定间隔执行动作：上滑固定 3 秒、10 秒、30 秒、50 秒
  - 开始上滑：使用页面选择的固定节奏，默认 3 秒，并记住上次选择
  - 开启 OCR
  - OCR 广告下一步阈值：100、200、300 三档，默认 100
  - 定位屏幕坐标
...
- 用户操作：
  用户打开操作页，选择 3 秒、10 秒、30 秒或 50 秒节奏后，点击“开始上滑”启动任务。
  任务运行过程中，双击悬浮窗可回到操作页并结束当前任务。
- 系统执行：
  任务启动后先等待 5 秒，再执行第一次上滑。
  每完成一次上滑后，按入口配置固定等待 3 秒、10 秒、30 秒或 50 秒进入下一轮，并持续循环。
  悬浮窗展示当前轮次剩余时间。
- 关键约束：
  打开操作页视为停止当前任务。
  “固定 3 秒/10 秒/30 秒/50 秒”指每次上滑后的下一轮等待，不和首次启动前的固定 5 秒等待混用。
  执行依赖无障碍服务可用。

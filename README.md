...
- 用户操作：
  用户打开操作页，点击第一个“开始上滑”启动 3 秒节奏任务，或点击第二个“开始上滑”启动 50 秒节奏任务。
  任务运行过程中，双击悬浮窗可回到操作页并结束当前任务。
- 系统执行：
  任务启动后先等待 5 秒，再执行第一次上滑。
  每完成一次上滑后，按入口配置固定等待 3 秒或 50 秒进入下一轮，并持续循环。
  悬浮窗展示当前轮次剩余时间。
- 关键约束：
  打开操作页视为停止当前任务。
  “固定 3 秒”和“固定 50 秒”指每次上滑后的下一轮等待，不和首次启动前的固定 5 秒等待混用。
  执行依赖无障碍服务可用。
  动作成功后默认不主动 toast，用户可直接从页面反馈观察执行结果；仅失败或异常时主动提示。
- 验收：
  点击“开始上滑”后任务能正常启动。
  首次上滑前有 5 秒等待。
  后续可以按对应入口的固定节奏持续上滑，并能看到倒计时变化。
  返回操作页后的行为符合预期。
...
- 用户操作：
  当前主页入口暂隐藏；能力保留在服务、悬浮窗和命令层，后续恢复入口后可显示独立连点悬浮按钮。
  连点悬浮按钮默认显示 `off`，点击后切换为 `on` 并开始连点，再点一次切回 `off` 并停止。
  连点悬浮按钮可拖动，用于调整点击锚点。
- 系统执行：
  连点开启后，每 50 ms 尝试执行一次点击。
  点击位置沿用原点击任务的悬浮窗锚点算法，即按连点悬浮按钮左上角位置加固定偏移计算。
  重新进入操作页会停止正在运行的连点，并把悬浮按钮状态更新为 `off`。
- 关键约束：
  连点是独立功能，不启动周期上滑、OCR 或坐标定位浮窗。
  50 ms 是调度间隔，实际点击落点和频率仍受系统无障碍手势调度影响。
  执行依赖无障碍服务和悬浮窗权限可用。
- 验收：
  点击“连点”后能显示独立悬浮按钮，默认 `off`。
  点击悬浮按钮后能切换 `on/off` 并开始或停止连点。
  拖动悬浮按钮后，后续连点位置随新的锚点变化。
  无障碍失效或回到操作页时，连点会停止。

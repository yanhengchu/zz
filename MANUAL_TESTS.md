...
- 前置条件：
  手动关闭本应用的无障碍服务。
- 操作：
  打开应用，然后在操作页点击“开始上滑”“开启 OCR”任一入口。
- 预期结果：
  打开应用本身不会立即跳转无障碍设置页。
...
- 前置条件：
  无障碍服务已开启。
- 操作：
  在操作页选择 3 秒、10 秒、30 秒或 50 秒任一节奏后，点击“开始上滑”。
- 预期结果：
...
- 前置条件：
  已按 [OCR_RULES.md](/Users/aschu/IdeaProjects/zz/OCR_RULES.md) 配置带 `num`、`CLICK`、`action_target`、`else_target` 的 `live_ad_next` 或 `aweme_ad_next` 规则（默认规则通常维护在 [ocr_rules_default.csv](/Users/aschu/IdeaProjects/zz/app/src/main/assets/ocr_rules_default.csv)，分辨率文件只覆盖差异字段），且页面上存在可识别的金币数字文案。
- 操作：
  在 OCR 按钮下方选择 100、200 或 300 后开启 OCR，分别观察一轮金币数低于阈值和高于阈值时的动作。
- 预期结果：
  应用首次启动时默认选择 100。
  当识别到的数字满足当前选择的 `GT` 阈值时，点击 `action_target`。
  当识别到的数字不满足当前选择的 `GT` 阈值时，点击 `else_target`。
